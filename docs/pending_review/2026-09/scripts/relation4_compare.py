...
D=int(sys.argv[1]); masses=None; POT=sys.argv[3] if len(sys.argv)>3 else '1/r'
if len(sys.argv)>2 and sys.argv[2]!='equal':
    ms=[sp.Rational(x) for x in sys.argv[2].split(',')]; masses={1:ms[0],2:ms[1],3:ms[2]}
alg=NBodyAlgebra(n_bodies=3,d_spatial=D,potential=POT,masses=masses)
