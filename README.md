...
> **Website:** [nbody.briansheppard.com](https://nbody.briansheppard.com) — interactive atlas explorer, curated figures, dataset browser, per-experiment status tracker, and the [neural-network universality article](https://nbody.briansheppard.com/article.html).
>
> **Interactive explainer:** [`explainer/`](explainer/) — a static, dependency-free scrollytelling walkthrough of the 11 technical terms in the [OEIS A395423](https://oeis.org/A395423) submission (orbit playground, pendulum portrait, bracket calculator, dimension growth, universality, SVD gap, frontier).
...
### Collision-edge syzygies and Levi-Civita / KS reading (Apr 2026)
...
[`collision_stratification.py`](collision_syzygy/collision_stratification.py). Full
writeup: [`COLLISION_SYZYGY_REPORT.md`](collision_syzygy/COLLISION_SYZYGY_REPORT.md). Exact
pipeline: [`collision_syzygy_v2.py`](collision_syzygy/collision_syzygy_v2.py); log:
[`syzygy_v2.log`](collision_syzygy/syzygy_v2.log). All collision/syzygy
scripts, JSON artifacts, and run logs live under
[`collision_syzygy/`](collision_syzygy/).
...
1. **Mass-invariant for generic masses**: the sequence [3, 6, 17, 116] is universal across potential types (1/r, 1/r², 1/r³, log, composite), spatial dimensions, and generic mass ratios. Mass invariance is proved symbolically for generic masses over ℚ(m₁,m₂,m₃): rank 116 holds for all mass triples outside a possible proper subvariety (none found; also confirmed numerically at 25+ mass ratios from 10⁻³ to 10⁶).
2. **Original survey artifact corrected** (Mar 23, 2026): the [3, 5, 13, 69] reported for unequal-mass gravitational systems was caused by SymPy 1.10.1 failing to lambdify level-3 expressions. Re-validation with SymPy 1.13.3 confirms [3, 6, 17, 116] for generic mass ratios.
3. **Charge invariance**: charge-invariant in everything tested — integer magnitudes q=1..20 in the mixed-sign (+1,+q,−1) sweep give exact rank 116 over ℚ, and same-sign configs (Penning trap, H₃⁺, O₃) also give 116. Early 500-sample runs reported Li⁺ (+3,−1,−1) → 111 and H₂⁺ (+1,+1,−1) → 115; revalidation at 1000–5000 samples (Mar 24, 2026) restored 116 in every case — undersampling artifacts, the same failure class as the SymPy-1.10 [3,5,13,69]. Levels 0–2 remain 3, 6, 17 universally.
...
† μ-sweep over μ ∈ {1/10, 1/2, 7/10, 1, 2, 5} (N=3, d=1, equal masses, max_level=3, 500 samples each), all [3, 6, 17, 116]; see [`results/yukawa_dimseq.json`](results/yukawa_dimseq.json). The named Yukawa configurations (tritium/He-3, dusty plasma, p-n-n) at their physical screening lengths and mass ratios are also complete (Apr 16, 2026), all [3, 6, 17, 116] — the full 9-config Yukawa survey is restored; see `docs/project_status.md`.
...
| `website/datasets.html` | Browser for the 13 Parquet tables (997 rows): sortable + filterable + paginated table view, schema block, per-table Plotly chart, JSON download + Hugging Face Parquet deep-link, cross-table filter panel |
...
- Schwarzschild composite (N=3, d=2): levels 0–3 give [3, 6, 17, 116]

GitHub Actions CI runs on pushes that touch the engines, registry, or tests (paths-filtered), and on every PR to `main`, testing Python 3.10 and 3.12. See [.github/workflows/regression.yml](.github/workflows/regression.yml).
