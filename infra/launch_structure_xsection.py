#!/usr/bin/env python3
"""
Launch EC2 instance for 1D structure cross-section.

Runs numerical structure constant sweep across mu for 1/r, r^4, and
1/r^4 potentials sequentially.

Usage:
    python launch_structure_xsection.py                    # on-demand
    python launch_structure_xsection.py --spot             # spot
    python launch_structure_xsection.py --dry-run          # preview
"""
...
TEMPLATE_PATH = os.path.join(os.path.dirname(__file__),
                             "userdata_structure_xsection.sh")
...
def launch_instance(instance_type, userdata, spot=False, dry_run=False):
    ...
    tag_name = "3body-structure-xsection"
...
def main():
    parser = argparse.ArgumentParser(
        description="Launch EC2 for 1D structure cross-section")
    ...
    results_path = "results/structure_cross_section"
    ...
    print("=" * 60)
    print("STRUCTURE CROSS-SECTION -- AWS LAUNCHER")
    print("=" * 60)
    print(f"  Task:          1D mu sweep for 1/r, r^4, 1/r^4 (200 pts each)")
    print(f"  Level:         2 (rank 17)")
    print(f"  Instance type: {args.instance_type}")
    print(f"  Market:        {'SPOT' if args.spot else 'on-demand'}")
    print(f"  Est. cost:     ~${hourly:.2f}/hr, "
          f"${hourly*1:.2f} (1hr) - ${hourly*3:.2f} (3hr)")
    ...
    if instance_id:
        ...
        print(f"    aws s3 sync s3://{S3_BUCKET}/{results_path}/ "
              f"./results/structure_cross_section/")
