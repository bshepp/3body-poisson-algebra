#!/bin/bash
# N=7 d=1 level 2: exact symbolic rank with per-level checkpointing
# Instance: r6i.4xlarge (16 vCPU, 128 GB RAM)
LOG=/var/log/3body-n7d1-level2.log
exec > "$LOG" 2>&1
set -x
set -o pipefail
ulimit -s unlimited
...
JOB_TAG="N7_d1_level2"
CHECKPOINT_DIR="$WORKDIR/nbody/checkpoints_n7d1_level2"
...
echo "=== N=7 d=1 Level-2 Symbolic Rank ==="
...
echo "=== Step 6: Running N=7 d=1 level 2 ==="
echo "CPUs: $NCPU, Workers: $WORKERS"
echo "Command: $PYTHON -u symbolic_rank_nbody.py -N 7 -d 1 --max-level 2 --checkpoint-dir $CHECKPOINT_DIR --workers $WORKERS"
echo "=== $(date -u) ==="
upload_log

cd "$WORKDIR/nbody"
$PYTHON -u symbolic_rank_nbody.py -N 7 -d 1 --max-level 2 \
...
echo "--- S3 rank JSON ---"
aws s3 ls "s3://$S3_BUCKET/$RESULTS_DIR/" | grep "N7_d1" 2>&1
