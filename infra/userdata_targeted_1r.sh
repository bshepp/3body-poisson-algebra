#!/bin/bash
# ==========================================================================
# AWS Spot Instance Launcher for Targeted Adaptive Scans — 1/r (Newtonian)
#
# Runs all 6 targeted regions for:
#   1. Reference 1/r (all-attractive)
#   2. Helium Coulomb 1/r (+2, -1, -1)
# ==========================================================================

LOG=/var/log/3body-targeted-1r.log
exec > "$LOG" 2>&1
set -x
...
echo "=== 3-Body Targeted Adaptive Scans (1/r Newtonian) ==="
...
shutdown_handler() {
    ...
    aws s3 cp "$LOG" "s3://$S3_BUCKET/results/targeted_1r_$(date +%s).log" || true
...
sync_logs() {
    while true; do
        sleep 60
        aws s3 cp "$LOG" "s3://$S3_BUCKET/results/targeted_1r_live.log" 2>/dev/null || true
        echo "{\"instance\": \"$INSTANCE_ID\", \"type\": \"$INSTANCE_TYPE\", \"potential\": \"1/r\", \"time\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\", \"uptime\": \"$(uptime)\"}" \
            | aws s3 cp - "s3://$S3_BUCKET/results/targeted_1r_heartbeat.json" 2>/dev/null || true
    done
}
sync_logs &
LOG_SYNC_PID=$!

# ---------- Run reference (uncharged) 1/r scans ---------------------------
echo ""
echo "=================================================================="
echo "=== Starting targeted scans: reference 1/r (Newtonian) ==="
echo "=== Started: $(date -u) ==="
echo "=================================================================="

python3 -u targeted_adaptive_scan.py \
    --potential 1/r \
    2>&1 | tee "$WORKDIR/targeted_1r_ref.log" &
SCAN_PID=$!
wait $SCAN_PID
REF_EXIT=$?
SCAN_PID=""

echo "=== Reference 1/r scan exit code: $REF_EXIT ==="

aws s3 sync "$WORKDIR/atlas_targeted/" "s3://$S3_BUCKET/atlas_targeted/" || true
aws s3 cp "$WORKDIR/targeted_1r_ref.log" "s3://$S3_BUCKET/results/targeted_1r_ref.log" || true
aws s3 cp "$LOG" "s3://$S3_BUCKET/results/targeted_1r_full.log" || true

# ---------- Run charged (helium) 1/r scans --------------------------------
echo ""
echo "=================================================================="
echo "=== Starting targeted scans: helium 1/r (Coulomb +2, -1, -1) ==="
echo "=== Started: $(date -u) ==="
echo "=================================================================="
...
python3 -u targeted_adaptive_scan.py \
    --potential 1/r \
    --charges 2 -1 -1 \
    2>&1 | tee "$WORKDIR/targeted_1r_chg.log" &
SCAN_PID=$!
wait $SCAN_PID
CHG_EXIT=$?
SCAN_PID=""

echo "=== Helium 1/r scan exit code: $CHG_EXIT ==="

aws s3 sync "$WORKDIR/atlas_targeted/" "s3://$S3_BUCKET/atlas_targeted/" || true
aws s3 cp "$WORKDIR/targeted_1r_chg.log" "s3://$S3_BUCKET/results/targeted_1r_chg.log" || true
aws s3 cp "$LOG" "s3://$S3_BUCKET/results/targeted_1r_full.log" || true

# ---------- Run analysis --------------------------------------------------
echo "=== Running analysis ==="
python3 -u targeted_adaptive_scan.py --analyze --potential 1/r 2>&1
python3 -u targeted_adaptive_scan.py --analyze --potential 1/r --charges 2 -1 -1 2>&1
...
cat > "$WORKDIR/completion_1r.json" <<CEOF
{
    "status": "complete",
    "potential": "1/r",
...
CEOF
aws s3 cp "$WORKDIR/completion_1r.json" "s3://$S3_BUCKET/results/targeted_1r_completion.json" || true
