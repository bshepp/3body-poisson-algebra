#!/bin/bash
# Symbolic Gram determinant and generator norm sweep
# Runs 1/r, r^4, and 1/r^4 potentials sequentially
LOG=/var/log/3body-gram-sweep.log
exec > "$LOG" 2>&1
set -x
set -o pipefail
ulimit -s unlimited
...
WORKDIR="/opt/3body"
RESULTS_DIR="results/symbolic_gram_sweep"
...
echo "=== Symbolic Gram Sweep ==="
echo "Started:  $(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "Instance: $INSTANCE_ID ($INSTANCE_TYPE)"

upload_log() {
    aws s3 cp "$LOG" "s3://$S3_BUCKET/$RESULTS_DIR/live.log" 2>/dev/null || true
    echo "{\"instance\": \"$INSTANCE_ID\", \"type\": \"$INSTANCE_TYPE\", \"job\": \"gram_sweep\", \"time\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\", \"uptime\": \"$(uptime)\"}" \
...
shutdown_handler() {
    ...
    aws s3 sync "$WORKDIR/nbody/results/symbolic_gram_sweep/" \
...
$PYTHON -m pip install sympy==1.13.3 numpy scipy gmpy2 mpmath
...
echo "Key files:"
ls -la "$WORKDIR/nbody/symbolic_gram_sweep.py" || echo "symbolic_gram_sweep.py NOT FOUND"
...
mkdir -p "$WORKDIR/nbody/results/symbolic_gram_sweep"
...
echo "=== Step 6a: Running 1/r Gram sweep ==="
echo "Command: $PYTHON -u symbolic_gram_sweep.py --potential 1/r --n-mu 200 --max-level 2"
echo "=== $(date -u) ==="
upload_log

$PYTHON -u symbolic_gram_sweep.py --potential "1/r" --n-mu 200 --max-level 2 &
MAIN_PID=$!
wait $MAIN_PID
EXIT_1R=$?
MAIN_PID=""
...
echo "=== Step 6b: Running r^4 Gram sweep ==="
echo "Command: $PYTHON -u symbolic_gram_sweep.py --potential r^4 --n-mu 200 --max-level 2"
echo "=== $(date -u) ==="
upload_log

$PYTHON -u symbolic_gram_sweep.py --potential "r^4" --n-mu 200 --max-level 2 &
MAIN_PID=$!
wait $MAIN_PID
EXIT_R4=$?
MAIN_PID=""
...
echo "=== Step 6c: Running 1/r^4 Gram sweep ==="
echo "Command: $PYTHON -u symbolic_gram_sweep.py --potential composite --composite -1 4 --n-mu 200 --max-level 2"
echo "=== $(date -u) ==="
upload_log

$PYTHON -u symbolic_gram_sweep.py --potential "composite" --composite "-1" "4" --n-mu 200 --max-level 2 &
MAIN_PID=$!
wait $MAIN_PID
EXIT_1R4=$?
MAIN_PID=""
...
cat > "/tmp/gram_sweep_completion.json" <<CEOF
{
    "status": "complete",
    "job": "symbolic_gram_sweep",
...
CEOF
aws s3 cp "/tmp/gram_sweep_completion.json" \
