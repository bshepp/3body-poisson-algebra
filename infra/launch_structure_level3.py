#!/usr/bin/env python3
"""
Launch EC2 instance for level-3 structure extraction.

Computes exact structure constants, Killing form, derived series, and
center for the rank-116 algebra at N=3, d=2, 1/r.

Usage:
    python launch_structure_level3.py                    # on-demand r6i.4xlarge
    python launch_structure_level3.py --spot             # spot (~70% cheaper)
    python launch_structure_level3.py --instance-type r6i.8xlarge  # more RAM
    python launch_structure_level3.py --dry-run           # preview
"""
...
def launch_instance(instance_type, userdata, spot=False, dry_run=False):
    ...
    tag_name = "3body-structure-level3"
...
def main():
    parser = argparse.ArgumentParser(
        description="Launch EC2 for level-3 structure extraction")
    parser.add_argument("--instance-type", default=DEFAULT_INSTANCE_TYPE,
                        help=f"EC2 instance type (default: {DEFAULT_INSTANCE_TYPE})")
    parser.add_argument("--spot", action="store_true",
                        help="Use spot instance (~70%% cheaper, risk of reclaim)")
    ...
    results_path = "results/algebra_structure/N3_d2_1r_level3"
    ...
    print("=" * 60)
    print("LEVEL-3 STRUCTURE EXTRACTION -- AWS LAUNCHER")
    print("=" * 60)
    print(f"  Task:          N=3 d=2 1/r --max-level 3 --structure")
    print(f"  Rank:          116 (6670 bracket pairs)")
    print(f"  Instance type: {args.instance_type}")
    print(f"  Market:        {'SPOT' if args.spot else 'on-demand'}")
    print(f"  Est. cost:     ~${hourly:.2f}/hr, "
          f"${hourly*6:.2f} (6hr) - ${hourly*24:.2f} (24hr)")
    ...
    if instance_id:
        ...
        print(f"    s3://{S3_BUCKET}/results/symbolic_rank/rank_N3_d2_1r.json")
        print(f"\n  Pull results:")
        print(f"    aws s3 sync s3://{S3_BUCKET}/{results_path}/ "
              f"./results/algebra_structure/N3_d2_1r_level3/")
