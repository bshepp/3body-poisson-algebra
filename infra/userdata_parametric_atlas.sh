#!/bin/bash
LOG=/var/log/3body-parametric-atlas.log
exec > "$LOG" 2>&1
set -x
set -o pipefail
ulimit -s unlimited
...
echo "=== Parametric Atlas Scan: $JOB_NAME ==="
echo "Started:  $(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "Instance: $INSTANCE_ID ($INSTANCE_TYPE)"
echo "Args:     $PARA_ARGS"

upload_log() {
    aws s3 cp "$LOG" "s3://$S3_BUCKET/results/parametric/${JOB_NAME}/live.log" 2>/dev/null || true
    echo "{\"instance\": \"$INSTANCE_ID\", \"type\": \"$INSTANCE_TYPE\", \"job\": \"$JOB_NAME\", \"time\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\", \"uptime\": \"$(uptime)\"}" \
        | aws s3 cp - "s3://$S3_BUCKET/results/parametric/${JOB_NAME}/heartbeat.json" 2>/dev/null || true
}
...
shutdown_handler() {
    ...
    echo "=== Uploading data on reclamation ==="
...
yum install -y python3 python3-pip python3-devel gcc gcc-c++ lapack-devel
...
echo "Key files:"
ls -la "$WORKDIR/parametric_atlas_scan.py" 2>/dev/null || echo "parametric_atlas_scan.py NOT FOUND"
...
echo "=== Step 6: Running parametric atlas scan ==="
echo "Command: $PYTHON -u $WORKDIR/parametric_atlas_scan.py $PARA_ARGS --output-dir $WORKDIR/atlas_full"
echo "=== $(date -u) ==="

cd "$WORKDIR"
$PYTHON -u parametric_atlas_scan.py $PARA_ARGS --output-dir "$WORKDIR/atlas_full" &
SCAN_PID=$!
...
upload_log
aws s3 cp "$LOG" "s3://$S3_BUCKET/results/parametric/${JOB_NAME}/full.log" || true

cat > "/tmp/parametric_completion.json" <<CEOF
{
    "status": "complete",
    "job": "$JOB_NAME",
    "para_args": "$PARA_ARGS",
...
CEOF
aws s3 cp "/tmp/parametric_completion.json" \
    "s3://$S3_BUCKET/results/parametric/${JOB_NAME}/aws_completion.json" || true
