...
def compute_killing_form(C):
    ...
    for i in range(r):
        ...
        for j in range(i, r):
            ad_j = C[j]  # (r, r)
            # C12 fix (2026-07): true Killing form is trace(ad_i @ ad_j), not
            # trace(ad_i @ ad_j.T) (that .T variant is the positive-semidefinite
            # ad-Gram/Frobenius form, a different and also-useful invariant).
            # Stored April 2026 killing_form.npy artifacts were produced by the
            # ad-Gram variant and are NOT regenerated by this fix.
            val = np.trace(ad_i @ ad_j)
            K[i, j] = val
            K[j, i] = val
